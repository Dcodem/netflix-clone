...
Optional trailer keys: copy `.env.example` to `.env.local` and fill `VITE_TMDB_API_KEY` / `VITE_IVA_API_KEY`, or paste them on the Account screen. `.env.local` is gitignored.
